...
class D3QN:
    def __init__(self, alpha, state_dim, action_dim, fc1_dim, fc2_dim, ckpt_dir,
                 ...
                 max_size=1000000, batch_size=256):# 0.95max_size=512, batch_size=64
        self.gamma = gamma
        self.tau = tau
    ...
    def choose_action(self, observation, isTrain=True):
        # 直接在 numpy 数组中添加批次维度，然后转换为张量
        state = T.tensor(observation[np.newaxis, :], dtype=T.float32).to(device)
    # def choose_action(self, observation, isTrain=True):
